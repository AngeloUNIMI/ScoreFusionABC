function [bestk,bestpp,bestmu,bestcov,dl,countf] = mixtures4(y, kmin, kmax, regularize, th, covoption, verb, plotta)
%
%
% Usage syntax:
...
while(k_cont)  % the outer loop will take us down from kmax to kmin components
    ...
    while(cont)    % this inner loop is the component-wise EM algorithm with the
        ...
        if (abs(deltlike/loglike(countf-1)) < th || isnan(abs(deltlike/loglike(countf-1))))
